...
return new class extends Migration
{
    public function up()
    {
        Schema::create('branding_requests', function (Blueprint $table) {
                $table->id(); // ID System (Auto Increment 1,2,3)
                
                // Kolom Data Sesuai Excel
                $table->date('submission_date')->nullable();
                $table->string('request_id')->nullable(); // ID dari Excel (BS750, dll)
                ...
                $table->string('request_type')->nullable(); // Opsional
                
                $table->string('jenis_tools_branding')->nullable();
                $table->text('ukuran_tools_branding')->nullable(); // String karena "337*135CM"
    ...
    {
        Schema::dropIfExists('branding_requests');
    }
};
